    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        <?php else: ?>
            ...
            <?php else:?>
                ...
                $stmt = $con->prepare("SELECT email,password,userID FROM oacademy WHERE email = ? AND password=?");
                ...
                if($rowEmail == $postEmail):
                    ...
                    if($rowPass == $postPass):
                        ...
                        //registre email & password in setcookies

                        $emailName = 'email';
                        $passwordName = 'password';
                        

                        setcookie($emailName, $rowEmail, time() + 86400, "/"); // 86400 = 1 day

                        setcookie($passwordName, $rowPass, time() + 86400, "/"); // 86400 = 1 day
                        
                        //echo $_COOKIE[$emailName];
                        //echo $_COOKIE[$passwordName];
                        
                        header('Location: ' . $inc . 'dashboard.php?pos=marketing&option=seo&id=' . $row['userID']);  //redirect to dashboard page
                        exit();
